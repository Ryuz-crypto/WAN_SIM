...
async function main() {
  ...
  try {
    ...
    await page.route('http://wansim-admin.test/**', async route => {
      ...
      if (url.pathname === '/health') return route.fulfill({ json: { ok: true, version: '2.0.11-stable', execution_mode: 'dry-run' } });
      ...
      if (url.pathname === '/api/v2/recovery/snapshots') return route.fulfill({ json: [{ id: 'snapshot-0001', configuration_id: 'config-0001', created_at: new Date().toISOString(), version: '2.0.11-stable', checksum: 'abc123', integrity: true, topology: 'nat' }] });
