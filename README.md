...
[![Versión estable](https://img.shields.io/badge/versi%C3%B3n-v2.0.12--rev3-00b388?style=flat-square)](https://github.com/Ryuz-crypto/WAN_SIM/releases/tag/v2.0.12-rev3)
...
**Versión estable recomendada y más actual:** [`v2.0.12-rev3`](https://github.com/Ryuz-crypto/WAN_SIM/tree/v2.0.12-rev3)
...
| --- | --- | --- |
| Instalación recomendada con FastAPI, ReactUI y rollback | `v2.0.12-rev3` | `sudo ./install-v2.sh install` |
...
V2.0.12 está validada por la matriz automática en estas versiones:
...
sudo apt-get update
sudo apt-get install -y git sudo
git clone --branch v2.0.12-rev3 --depth 1 https://github.com/Ryuz-crypto/WAN_SIM.git WAN_SIM-v2
cd WAN_SIM-v2
...
## Novedades De V2.0.12 Rev3

- **El plan Bridge ya no deshabilita el forwarding IPv4 global.** La acción `forwarding-disable` apagaba `net.ipv4.ip_forward` en todo el host, lo que interrumpía el enrutado de Docker y cortaba el acceso por red al plano de control (ReactUI/API) aunque el host siguiera respondiendo ping y SSH. El Bridge L2 no requiere forwarding; el NAT lo activa solo cuando corresponde.
- Los bridges L2 (`br_wan*`) se crean ahora con **STP activado** y `verify` exige que permanezca activo: evita tormentas de broadcast cuando los dos puertos de un par terminan en el mismo segmento L2.
- La aceptación Vagrant valida `stp_state` en los bridges reales antes de aprobar el despliegue Bridge.

Rev2 de V2.0.12 incorporó:

- STP activado en los bridges administrados.

Rev1 de V2.0.12 incorporó:
