        ...
        if [ "$CONFIG_DHCP" = "s" ]; then
            ...
            while true; do
                ...
                case "$SEGMENT_OPTION" in
                    ...
                    *) log_message "ERROR" "Opción inválida. Ingresa 1, 2 o 3." ;;
                esac
            done
...
# SECCIÓN 6.2: Configuración y Despliegue del Servicio Flask
# --------------------------------------------------------
log_message "INFO" "Configurando y desplegando el servicio Flask..."

# Reemplazar placeholders en el archivo Python
log_message "DEBUG" "Reemplazando placeholders en $WANSIM_DASHBOARD..."
for placeholder in TELEGRAM_TOKEN TELEGRAM_CHAT_ID TELEGRAM_ENABLED PYTHON_VLAN_LIST; do
    value=$(eval echo "\$$placeholder")
    if ! sed -i "s|__${placeholder}__|$value|g" "$WANSIM_DASHBOARD"; then
        log_message "ERROR" "No se pudo reemplazar $placeholder en $WANSIM_DASHBOARD."
        echo "${COLOR_ERROR}No se pudo modificar $WANSIM_DASHBOARD. Revisa permisos con 'ls -l $WANSIM_DASHBOARD'.${COLOR_RESET}"
        exit 1
    fi
done
log_message "OK" "Placeholders reemplazados en $WANSIM_DASHBOARD."
...
free_port() {
    ...
    local pid
    if command -v ss >/dev/null 2>&1; then
        pid=$(ss -tuln | grep ":$port" | awk '{print $NF}' | head -n 1)
    elif command -v lsof >/dev/null 2>&1; then
    ...
    if [ -n "$pid" ]; then
        log_message "INFO" "Puerto $port en uso por PID $pid. Intentando terminar..."
        sudo kill -15 "$pid" 2>/dev/null
        sleep 1
        if kill -0 "$pid" 2>/dev/null; then
            log_message "ADVERTENCIA" "PID $pid no terminó. Forzando terminación..."
    ...
    if ! ss -tuln | grep -q ":$port"; then
        log_message "OK" "Puerto $port liberado."
    else
...
SERVICE_PYTHONPATH="$PYTHONPATH"
if [ -n "$USER_SITE" ]; then
    SERVICE_PYTHONPATH="$USER_SITE:$SERVICE_PYTHONPATH"
fi
...
Environment=PYTHONPATH=$SERVICE_PYTHONPATH
...
def parse_qdisc(output):
    ...
    delay = jitter = loss = 0
    try:
        # Buscar delay y jitter en el formato "delay Xms Yms"
        ...
        # Buscar solo delay si no hay jitter
...
TELEGRAM_ENABLED = __TELEGRAM_ENABLED__
    ...
    if not VLAN_INTERFACES:
        ...
        return render_template_string("""
            <div class='alert alert-warning text-center'>
                No hay VLANs configuradas. Por favor, verifica la configuración en /home/axis/emix_abundix.conf.
            </div>
        """), 400
    stats = {}
    for vlan in VLAN_INTERFACES:
...
def configure():
    ...
    try:
        ...
        if not VLAN_INTERFACES:
            ...
            error_message = "No hay VLANs configuradas. Verifica /home/axis/emix_abundix.conf."
...
Ancho de Banda Salida: {{ stats[vlan][current_unit + '_out']|round(2) }} {{ current_unit }}</pre>
        ...
        logger.error(f"Error en /configure: {str(e)}")
