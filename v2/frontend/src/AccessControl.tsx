...
export function AccessControl({ identity, onNotice, onLogout }: Props) {
  ...
  const remove = async (user: User) => {
    if (!window.confirm(`Eliminar permanentemente al usuario ${user.username}? Sus sesiones se cerrarán.`)) return
    try { await api.deleteUser(user.id); onNotice(`Usuario ${user.username} eliminado.`); await refresh() }
    catch (error) { onNotice(error instanceof Error ? error.message : 'No se pudo eliminar el usuario.') }
  }
  const rotate = async () => {
  ...
  return <section className="workspace access-grid">
    ...
    <section className="panel"><div className="panel-heading"><div><span className="eyebrow">Sesiones</span><h2>Usuarios</h2></div><ShieldCheck size={22} /></div><div className="user-list">{users.map(user => <div key={user.id}><div><strong>{user.username}</strong><span>{user.enabled ? 'Habilitado' : 'Deshabilitado'}</span></div><select aria-label={`Rol de ${user.username}`} value={user.role} onChange={event => void update(user, { role: event.target.value as UserRole })}><option value="viewer">viewer</option><option value="operator">operator</option><option value="admin">admin</option></select><div className="button-row inline"><button className="secondary-button compact" onClick={() => void update(user, { enabled: !user.enabled })}>{user.enabled ? 'Deshabilitar' : 'Habilitar'}</button><button className="icon-button danger" disabled={identity.id === user.id} title={identity.id === user.id ? 'No puedes eliminar tu sesión actual' : `Eliminar ${user.username}`} onClick={() => void remove(user)}><Trash2 size={16} /></button></div></div>)}</div></section></>}
