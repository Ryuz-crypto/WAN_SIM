...
import {
  Cable, CheckCircle2, Clock3, Gauge, GitCompareArrows, LayoutDashboard, Network,
  Play, RefreshCw, RotateCcw, Router, Save, ServerCog, ShieldCheck, SlidersHorizontal, Workflow, X,
} from 'lucide-react'
import { api } from './api'
import type { Config, Configuration, Deployment, L3Link, Overview } from './types'
...
function App() {
  const [tab, setTab] = useState<'configure' | 'operations' | 'audit'>('configure')
  ...
  const refresh = async () => {
    try {
      const [nextOverview, nextDeployments, nextHealth] = await Promise.all([api.overview(), api.deployments(), api.health()])
      setOverview(nextOverview); setDeployments(nextDeployments); setHealth(nextHealth)
