...
function App() {
  ...
  const deploy = async (confirmation?: string) => {
    if (!configuration) return
    setBusy(true); setNotice('')
    try { const result = await api.deploy(configuration.id, true, confirmation); setDeployments(value => [result, ...value]); setNotice(`Despliegue ${result.status.toLowerCase()}.`); setConfirmDeployment(false); setConfirmationText(''); await refresh() }
    catch (error) { setNotice(error instanceof Error ? error.message : 'No se pudo desplegar.') }
    finally { setBusy(false) }
  }
  const requestDeployment = () => {
    if (!configuration) return
    if (health?.execution_mode === 'host') { setConfirmationText(''); setConfirmDeployment(true); return }
    void deploy()
  }
  ...
  return <main className="app-shell">
    ...
    {confirmDeployment && configuration && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="deployment-confirm-title"><section className="modal-panel"><div className="panel-heading"><div><span className="eyebrow">Modo host</span><h2 id="deployment-confirm-title">Confirmar cambios reales</h2></div><ShieldCheck size={23} /></div><p>Se aplicarán las acciones mostradas al host. Para continuar escribe:</p><code className="confirmation-code">APLICAR {configuration.id}</code><label>Confirmación<input autoFocus value={confirmationText} onChange={event => setConfirmationText(event.target.value)} /></label><div className="button-row"><button className="secondary-button" onClick={() => { setConfirmDeployment(false); setConfirmationText('') }}>Cancelar</button><button className="primary-button" disabled={busy || confirmationText !== `APLICAR ${configuration.id}`} onClick={() => void deploy(confirmationText)}><Play size={17} />Aplicar cambios</button></div></section></div>}
    </>}
  </main>
}
